...
import { FaPlus, FaCreditCard, FaMoneyBillWave, FaShieldAlt, FaChevronLeft, FaChevronRight, FaSearch } from 'react-icons/fa'
...
const Financials = () => {
  ...
  const createPayment = async () => {
    ...
    try {
      // إرسال المعاملة مع بيانات التحديث المالي للمريض للباك-أند
      const { data } = await axios.post(`${API_URL}/createPayment`, {
        ...
        description,
        method,
        patientId: type === 'income' ? selectedPatient?.id : null, // إرسال الـ ID لتحديث ملفه المالي
      ...
      if (data.error === false) {
        ...
        fetchPatients(); // تحديث أرقام المرضى محلياً لرؤية الخصم الفوري
      }
    } catch (error) {
  ...
  const [servicesList, setServicesList] = useState([]); // لتخزين قائمة الخدمات من الباك إند
const [selectedService, setSelectedService] = useState(''); // لتخزين الخدمة المختارة

useEffect(() => {
  const fetchServices = async () => {
    try {
      // استبدل الرابط برابط السيرفر الفعلي لديك
  ...
  // جلب الحركات المادية وقائمة المرضى عند تحميل الصفحة
  useEffect(() => {
  ...
  <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 sm:gap-8 px-4 sm:px-8">
            ...
            <table className="w-full text-right border-collapse min-w-[600px]">
              <thead>
                <tr className="border-b border-slate-100 text-slate-400 text-xs sm:text-sm">
                  ...
                  <th className="pb-3 font-medium">الإجراء / الخدمة</th>
    ...
    <div className="bg-white border border-slate-100 rounded-2xl shadow-sm p-5 sm:p-6 h-fit overflow-visible">
      ...
      <div className="space-y-4">
        ...
        {/* حقل اسم المريض الذكي أو اسم الجهة الموردة */}
